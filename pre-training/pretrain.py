...
def main():
    ...
    parser.add_argument("--config_path", type=str, default="assets/bert/base_config.json",
    ...
    # Contrastive learning options (pretraining-objective ablation).
    parser.add_argument("--cl_loss", choices=["none", "cl_only", "mlm_cl"], default="none",
                        help="Pretraining objective: none = MLM-only; cl_only = contrastive "
                             "only; mlm_cl = MLM + contrastive (Eq. 10 of Flow-BERT). "
                             "Requires a class-labeled dual dataset.")
    parser.add_argument("--cl_alpha", type=float, default=0.5,
                        help="Weight of same-class positive pairs in the CL loss (Eq. 9).")
    parser.add_argument("--cl_beta", type=float, default=0.5,
                        help="Weight of cross-class negative pairs in the CL loss (Eq. 9).")
    parser.add_argument("--cl_margin", type=float, default=1.0,
                        help="Margin m for negative pairs in the CL loss (Eq. 9).")
    parser.add_argument("--lambda_mlm", type=float, default=1.0,
                        help="Weight of the MLM loss in the joint objective (Eq. 10).")
    parser.add_argument("--lambda_cl", type=float, default=1.0,
                        help="Weight of the CL loss in the joint objective (Eq. 10).")
    ...
    if args.encoder == "dual_stream_cross":
        if args.target != "bert":
            raise ValueError(
                "Pretraining with --encoder dual_stream_cross requires --target bert "
                "(dual-stream split MLM; no NSP)."
            )
        args.dual_stream_pretrain = True
        args.dual_mlm_only = True
        if args.cl_loss == "cl_only" or args.cl_loss == "mlm_cl":
            # CL needs the class-labeled batch branch (7-tuple) and the joint
            # loss combination handled by the trainer.
            pass
    else:
        args.dual_stream_pretrain = False
        args.dual_mlm_only = False
