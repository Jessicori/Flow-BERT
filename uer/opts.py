def model_opts(parser):
    ...
    parser.add_argument("--encoder", choices=["transformer", "dual_stream_cross", "rnn", "lstm", "gru",
                                              "birnn", "bilstm", "bigru",
                                              "gatedcnn"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="dual_stream_cross: number of macro blocks; each block is dual_self_layers_per_block "
        "self steps per stream then one cross-attention (default 3 blocks => 3 cross layers).",
    )
    parser.add_argument(
        "--dual_self_layers_per_block",
        type=int,
        default=4,
        help="dual_stream_cross: TransformerLayer pairs (flow+payload) per stream inside each block before cross.",
    )
...
def finetune_opts(parser):
    ...
    parser.add_argument("--config_path", default="assets/bert/base_config.json", type=str,
    ...
    parser.add_argument("--flow_seq_length", type=int, default=32,
                        help="Flow stream padded length when using dual_stream_cross (<= max_seq_length).")
    parser.add_argument("--payload_seq_length", type=int, default=128,
                        help="Payload stream padded length when using dual_stream_cross (<= max_seq_length).")
...
def infer_opts(parser):
    ...
    parser.add_argument("--seq_length", type=int, default=128,
                        help="Sequence length.")
    parser.add_argument("--flow_seq_length", type=int, default=32,
                        help="Flow stream padded length when using dual_stream_cross.")
    parser.add_argument("--payload_seq_length", type=int, default=128,
                        help="Payload stream padded length when using dual_stream_cross.")
