...
class DataLoader(object):
    ...
    def __del__(self):
        reader = getattr(self, "dataset_reader", None)
...
class DualStreamBertDataLoader(DataLoader):
    """
    Batches dual-stream BERT pretraining instances from ``Tor_preprocess_dual.py``:
    static: (src_flow, tgt_mlm_flow, src_payload, tgt_mlm_payload, mlm_side)
    dynamic: (src_flow, src_payload, mlm_side) — mask only the chosen stream here.
    Yields: src_flow, src_payload, tgt_mlm_flow, tgt_mlm_payload, seg_flow, seg_payload
    """
    ...
    def __iter__(self):
        while True:
            ...
            for ins in instances:
                if len(ins) == 5:
                    src_f, mlm_f, src_p, mlm_p, mlm_side = ins
                    masked_words_num += len(mlm_f) + len(mlm_p)
                    lf, lp = len(src_f), len(src_p)
                    row_f = [0] * lf
                    row_p = [0] * lp
                    for pos, tid in mlm_f:
                        row_f[pos] = tid
                    for pos, tid in mlm_p:
                        row_p[pos] = tid
                    src_flow_batch.append(src_f)
                    src_payload_batch.append(src_p)
                    tgt_flow_batch.append(row_f)
                    tgt_payload_batch.append(row_p)
                    seg_flow_batch.append([1 if t != PAD_ID else 0 for t in src_f])
                    seg_payload_batch.append([1 if t != PAD_ID else 0 for t in src_p])
                elif len(ins) == 6:
                    src_f, mlm_f, src_p, mlm_p, mlm_side, lab = ins
                    masked_words_num += len(mlm_f) + len(mlm_p)
                    lf, lp = len(src_f), len(src_p)
                    row_f = [0] * lf
                    row_p = [0] * lp
                    ...
                    seg_flow_batch.append([1 if t != PAD_ID else 0 for t in src_f])
                    seg_payload_batch.append([1 if t != PAD_ID else 0 for t in src_p])
                    label_batch.append(int(lab))
                elif len(ins) == 3:
                    src_f, src_p, mlm_side = ins
                    lf, lp = len(src_f), len(src_p)
                    row_f = [0] * lf
                    row_p = [0] * lp
                    if mlm_side == 0:
                        src_f, mlm_f = mask_seq(
                            list(src_f),
                            self.tokenizer,
                        ...
                        for pos, tid in mlm_p:
                            row_p[pos] = tid
                    masked_words_num += sum(1 for x in row_f if x > 0) + sum(1 for x in row_p if x > 0)
                    src_flow_batch.append(src_f)
                    src_payload_batch.append(src_p)
                    tgt_flow_batch.append(row_f)
                    tgt_payload_batch.append(row_p)
                    ...
                    raise ValueError(
                        "DualStreamBertDataLoader expects instance length 5 (static MLM) or 3 (dynamic)."
                    )
            ...
            if label_batch:
                assert len(label_batch) == len(src_flow_batch), "mixed labeled/unlabeled instances"
                yield (
                    torch.LongTensor(src_flow_batch),
                    torch.LongTensor(src_payload_batch),
                    torch.LongTensor(tgt_flow_batch),
                    torch.LongTensor(tgt_payload_batch),
                    torch.LongTensor(seg_flow_batch),
                    torch.LongTensor(seg_payload_batch),
